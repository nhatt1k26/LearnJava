...
select
	 m.*, UnitsBalance, UnitCost,DateKey 
into 
	LatestStockInformation
from 
	ProductInformation m
join
	(
	select a.ProductKey,a.DateKey,a.UnitsBalance,a.UnitCost
	from 
		FactProductInventory a
	join
	(
		select ProductKey,Max(DateKey) as MaxDate
		from
			FactProductInventory
		group by ProductKey
	) latest
	on 
		a.ProductKey = latest.ProductKey and a.DateKey = latest.MaxDate
	) latestinfo
on
...
select
	ProductName,ModelName,ProductLine,ProductCategoryName,ProductSubCategoryName,DealerPrice,ListPrice,Color,Decription, m.UnitsBalance, m.UnitCost
from 
	LatestStockInformation m
join
	(
